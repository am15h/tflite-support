...
#include "third_party/tensorflow_lite_support/cc/task/vision/object_detector.h"
...
#include "third_party/tensorflow/lite/experimental/support/metadata/metadata_schema_generated.h"
...
#include "third_party/tensorflow_lite_support/cc/task/core/task_utils.h"
...
#include "third_party/tensorflow_lite_support/cc/task/vision/utils/frame_buffer_utils.h"
#include "third_party/tensorflow_lite_support/cc/task/vision/utils/image_tensor_specs.h"
...
using ::tflite::support::task::core::TfLiteEngine;
...
StatusOr<std::unique_ptr<ObjectDetector>> ObjectDetector::CreateFromOptions(
  ...
  auto engine = absl::make_unique<TfLiteEngine>();
  auto frame_buffer_utils =
      FrameBufferUtils::Create(FrameBufferUtils::ProcessEngine::kLibyuv);

  auto object_detector = absl::make_unique<ObjectDetector>(
      std::move(engine), std::move(frame_buffer_utils));

  RETURN_IF_ERROR(object_detector->Init(options));

  return object_detector;
}

absl::Status ObjectDetector::Init(const ObjectDetectorOptions& options) {
  // Copy options.
  options_ = options;

  // TODO(b/159774988): extend and use TaskApiFactory.
  // Initialize engine and interpreter.
  ASSIGN_OR_RETURN(model_file_handler_,
                   ExternalFileHandler::CreateFromExternalFile(
                       &options_.model_file_with_metadata()));
  absl::string_view flat_buffer = model_file_handler_->GetFileContent();
  RETURN_IF_ERROR(engine_->BuildModelFromFlatBuffer(flat_buffer.data(),
                                                    flat_buffer.length()));
  RETURN_IF_ERROR(engine_->InitInterpreter());
