...
#include "third_party/tensorflow_lite_support/cc/task/vision/utils/frame_buffer_common_utils.h"
#include "third_party/tensorflow_lite_support/cc/task/vision/utils/libyuv_frame_buffer_utils.h"
...
BoundingBox OrientBoundingBox(const BoundingBox& from_box,
  ...
  FrameBuffer::Dimension rotate_from_dimension = from_dimension;
  if (params.rotation_angle_deg == 90 || params.rotation_angle_deg == 270) {
    rotate_from_dimension.Swap();
  }
  ...
  if (params.flip == OrientParams::FlipType::kVertical) {
    to_box.set_origin_y(rotate_from_dimension.height -
                        (to_box.origin_y() + to_box.height()));
  }
  if (params.flip == OrientParams::FlipType::kHorizontal) {
    to_box.set_origin_x(rotate_from_dimension.width -
...
OrientParams GetOrientParams(FrameBuffer::Orientation from_orientation,
  ...
  DCHECK(from_index > -1 && to_index > -1);
