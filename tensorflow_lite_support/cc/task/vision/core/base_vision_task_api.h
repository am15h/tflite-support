...
class BaseVisionTaskApi
 ...
 public:
  explicit BaseVisionTaskApi(
      std::unique_ptr<core::TfLiteEngine> engine,
      std::unique_ptr<FrameBufferUtils> frame_buffer_utils)
      : tflite::support::task::core::BaseTaskApi<OutputType, const FrameBuffer&,
                                                 const BoundingBox&>(
            std::move(engine)),
        frame_buffer_utils_(std::move(frame_buffer_utils)) {}
